...
export class UsersService implements OnModuleInit {
    ...
    try {
      ...
      this.users = JSON.parse(data).reduce((users, rawUser: User) => {
        const userData = Object.keys(rawUser).reduce((user, key) => {
          let data = rawUser[key];
          if (key === 'homeAddress'|| key === 'notificationAddress') {
  ...
  public async updateUserData(userId: string, userData: UpdateUserDataBody): Promise<User> {
    ...
    const { firstName, lastName, homeAddress, notificationAddress } = userData;
  ...
  public async assignToHouse(userId: string, houseId: HouseId): Promise<User> {
    const user = this.findById(userId);
    if (!user) {
  ...
  public findUserByHouseId(houseId: HouseId): User[] {
    return Array.from(this.users.values()).filter(({ house }) => house === houseId);
  }
